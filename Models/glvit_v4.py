...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        B, M, C = x.shape
        qk = self.qk(x)  # Shape: (B, N, 2 * dim)
        qk = rearrange(qk, "b n (qk h d) -> qk b h n d", qk=2, h=self.num_heads, d=self.head_dim)
        q, k = qk
        N= (M-2)//2
        # print(N)
        # print(M)
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=32, patch_size=4, in_channels=3, num_classes=10, dim=64, depth=6, heads=8, mlp_dim=128, dropout=0.1):
        ...
        self.w_cls = nn.Parameter(torch.randn(2))
    ...
    def forward(self, x):
        ...
        combined_cls_output = self.w_cls[0]*cls_output1 + self.w_cls[1]*cls_output2  # Might change later
